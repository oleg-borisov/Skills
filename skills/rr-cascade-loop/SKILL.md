---
name: rr-cascade-loop
description: Последовательно реализовать готовые задачи спецификации с совмещённым review исправлений и следующей задачи.
...
LEDGER = .scratch/rr-cascade-loop/<branch>.md. Храни:
...
Запусти fresh implementer со spec paths, acceptance criteria текущей задачи, task BASE и scope: текущая задача плюс неустранённые carried critical findings и human FIX_NOW, с IDs. Будущие задачи вне scope. Потребуй per-finding disposition дополнительно к commit SHA, changed files, targeted checks и risks. Проверь, что HEAD совпадает с отчётом.
...
- Нет active BLOCKER/MAJOR → на промежуточной задаче сохрани MINOR/human items до финального Human decisions, отметь задачу REVIEWED и перейди к Task activation следующей; на последней обработай MINOR/human items.
- Есть active critical findings → Revise.
- Тот же critical finding без прогресса в двух последовательных review сквозь задачи или четыре critical review iterations текущей задачи → HUMAN_ATTENTION. Противоречие, требующее человека, также останавливает переход.
...
Запусти fresh reviser с critical findings, указаниями человека и eligible cheap MINOR. Прими commit SHA либо подтверждение unchanged HEAD, per-finding disposition и targeted checks. На последней задаче отклонённый BLOCKER всегда переводи в HUMAN_ATTENTION.

Для промежуточной задачи после отчёта reviser, в том числе при unchanged HEAD или неисправленном critical finding, запиши заявленные исправления как AWAITING_REVIEW, остальные оставь открытыми; state задачи = AWAITING_REVIEW. Передай оставшиеся findings следующему Implement через Task activation, без отдельных checks/delta-review. Явный отказ человека исправлять critical finding или HUMAN_ATTENTION останавливает переход.

На последней задаче: при новом commit запусти delta Review только по originating axes. При unchanged HEAD не запускай checks или review.
